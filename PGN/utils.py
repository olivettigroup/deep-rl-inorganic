...
def get_fp(smiles, singular=True):
    ...
    if singular:
        for i in range(len(smiles)):
            ...
            mol = smiles[i]
            try:
                tmp = StrToComposition().featurize(mol)
                tmp = feature_calculators.featurize(tmp[0])
                fp.append(np.array(tmp))
                processed_indices.append(i)
            except:
                invalid_indices.append(i)
        fp = np.array(fp)
    else:
        exceptions = ['StrToComposition Exceptions',
        'Stoichiometry Exceptions',
        'ElementProperty Exceptions',
        'ValenceOrbital Exceptions',
        'IonProperty Exceptions']

        df = pd.DataFrame()
        df["comp"] = smiles
        df = StrToComposition(target_col_id="composition_obj").featurize_dataframe(df, col_id="comp", ignore_errors=True, return_errors=True)
        df = feature_calculators.featurize_dataframe(df, col_id="composition_obj", ignore_errors = True)

        for col in exceptions:
            invalid_indices.extend(df[~np.isnan(df[col])].index)
            processed_indices.extend(df[np.isnan(df[col])].index)
        ...
        df = df.drop(columns=["comp", "composition_obj"])
        df = df.drop(columns = exceptions)
        df = df[processed_indices]

        fp = df.values
    return fp, processed_indices, invalid_indices
