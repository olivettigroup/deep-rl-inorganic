...
class Reinforcement(object):
    def __init__(self, generator, predictor, get_reward, get_uniqueness_reward):
        """
        ...
        self.get_uniqueness_reward = get_uniqueness_reward
    ...
    def policy_gradient(self, data, n_batch=10, gamma=0.97,
        ...
        """
        rl_loss = 0
        uniqueness_rl_loss = 0
        self.generator.optimizer.zero_grad()
        total_reward = 0
        # total_uniqueness_reward = 0
        # trajectories_for_reward = []
        for _ in range(n_batch):
            ...
            trajectory = '<>'
            while reward == "start":
                trajectory = self.generator.evaluate(data, predict_len=5)
                if std_smiles:
                    try:
                    ...
                    # remove starting '<'
                    trajectory_for_reward = trajectory[1:]
                    if len(trajectory_for_reward) > 0:
                        # remove ending '>'
                        if trajectory_for_reward[-1] == '>':
                            trajectory_for_reward = trajectory_for_reward[:-1]
                    reward = self.get_reward(trajectory_for_reward,
                                             self.predictor,
                                             **kwargs)
            # trajectories_for_reward.append(trajectory_for_reward)
            ...
            for p in range(len(trajectory_input)-1):
                ...
                # added
                # uniqueness_rl_loss -= (log_probs[0, top_i] * gamma**p)

        
        # add uniqueness reward
        # uniqueness_reward = self.get_uniqueness_reward(trajectories_for_reward)
        # total_uniqueness_reward = uniqueness_reward * n_batch
        # total_uniqueness_reward *= 10
        # total_reward += total_uniqueness_reward
        # uniqueness_rl_loss *= uniqueness_reward

        # uniqueness_rl_loss *= 10


        # rl_loss += uniqueness_rl_loss
