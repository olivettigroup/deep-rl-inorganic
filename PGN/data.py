...
class GeneratorData(object):
    ...
    def __init__(self, training_data_path, tokens=None, start_token='<', 
        ...
        for i in range(len(data)):
            len_data = len(Composition(data[i]).get_el_amt_dict())
    ...
    def char_tensor(self, string):
        ...
        """
        #char_seq = re.findall(r"[A-Z][a-z]?|[0-9]|>|<|.", string)
        # char_seq = re.findall(r"[A-Z][a-z]?|[0-9]|>|<", string)
        char_seq = re.findall(r"[A-Z][a-z]?[0-9]|>|<", string)
        tensor = torch.zeros(len(char_seq)).long()
        for c in range(len(char_seq)):
            tensor[c] = self.all_characters.index(char_seq[c])
        if self.use_cuda:
